...
namespace ROYcms.Ask.DAL
{
	...
	public class ROYcms_question
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("bh", "ROYcms_question"); 
		}
		...
		public bool Exists(int bh)
		{
			...
			strSql.Append("select count(1) from ROYcms_question");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into ROYcms_question(");
			strSql.Append("title,content,tag,status,reward,user_id,type_id,hits,star_time,end_time,ip,switchs)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update ROYcms_question set ");
			...
			parameters[7].Value = model.type_id;
			parameters[8].Value = model.hits;
			parameters[9].Value = model.star_time;
			parameters[10].Value = model.end_time;
			parameters[11].Value = model.ip;
			parameters[12].Value = model.switchs;
		...
		public void Delete(int bh)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ROYcms_question ");
			strSql.Append(" where bh=@bh ");
			SqlParameter[] parameters = {
					new SqlParameter("@bh", SqlDbType.Int,4)};
			parameters[0].Value = bh;
		...
		public ROYcms.Ask.Model.ROYcms_question GetModel(int bh)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 bh,title,content,tag,status,reward,user_id,type_id,hits,star_time,end_time,ip,switchs from ROYcms_question ");
			strSql.Append(" where bh=@bh ");
			SqlParameter[] parameters = {
					new SqlParameter("@bh", SqlDbType.Int,4)};
			parameters[0].Value = bh;

			ROYcms.Ask.Model.ROYcms_question model=new ROYcms.Ask.Model.ROYcms_question();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["reward"].ToString()!="")
				{
					model.reward=int.Parse(ds.Tables[0].Rows[0]["reward"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select bh,title,content,tag,status,reward,user_id,type_id,hits,star_time,end_time,ip,switchs ");
			strSql.Append(" FROM ROYcms_question ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" bh,title,content,tag,status,reward,user_id,type_id,hits,star_time,end_time,ip,switchs ");
			strSql.Append(" FROM ROYcms_question ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "ROYcms_question";
