...
// 编辑模型是的 权限列表赋值
if($("#Role").length > 0){
	for (i=0;i<document.InsertObjetForm.Role.options.length;i++)
	{ 
		 if(document.InsertObjetForm.Role.options[i].value==$("#RoleVal").val())
		 {
			 document.InsertObjetForm.Role.options[i].selected=true;break;
		 }
	}
	//可见状态赋值
	for (i=0;i<document.InsertObjetForm.Visible.options.length;i++)
	{ 
		 if(document.InsertObjetForm.Visible.options[i].value==$("#VisibleVal").val())
		 {
			 document.InsertObjetForm.Visible.options[i].selected=true;break;
		 }
	}
}
...
// 编辑管理员用户是的 权限列表赋值
if($("#Rol").length > 0){
	for (i=0;i<document.Edit_AdminUserForm.Rol.options.length;i++)
	{ 
		 if(document.Edit_AdminUserForm.Rol.options[i].value==$("#RolVal").val())
		 {
			 document.Edit_AdminUserForm.Rol.options[i].selected=true;break;
		 }
	}
}
