...
{
    [DefaultProperty("")]
    [ToolboxData("<{0}:XmlOrder  runat=server />")]
    public class XmlOrder : Literal
    {
        ...
        protected override void Render(HtmlTextWriter output)
        {
            try
            {
                ...
                XMLCon = GetValue(this.Rid, this.Name);
        ...
        public static string GetValue(string ID, string name)
        {
            try
            {
                ROYcms.Common.XmlControl XmlControl = new ROYcms.Common.XmlControl(HttpContext.Current.Server.MapPath("~/APP_XML/Order/" + ID + ".xml"));
                return XmlControl.GetText("ROYcms/Order/" + name);
            }
            catch
            {
                return "";
