...
{
    public partial class ASKAdmin : ROYcms.AdminPage
    {
        public ROYcms.Sys.BLL.ROYcms_ASK Bll = new Sys.BLL.ROYcms_ASK();
        public ROYcms.Sys.Model.ROYcms_ASK Model = new Sys.Model.ROYcms_ASK();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                int Id = ROYcms.Common.Request.GetQueryInt("Id");
                int State = ROYcms.Common.Request.GetQueryInt("State");
                if (State == -1)
                {
