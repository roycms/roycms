...
/// 
/// </summary>
    public partial class template_Default : AdminPage
    {  
        #region type属性
        /// <summary>
        /// 
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    BindGrid(Server.MapPath(ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_root") + ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_file")));

                }
                catch { BindGrid(Server.MapPath(ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_root"))); }
        ...
        {
            if (!path.Contains(Server.MapPath(ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_root"))))
            {
                path = Server.MapPath(ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_root"));
            }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else
            {
                ...
                path = path.Replace("\\", "/");
                path = path.Replace("//", "/");
                int al1 = path.LastIndexOf((ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_root")).Replace("~", ""));
                int al2 = path.Length - path.LastIndexOf((ROYcms.Config.ROYcmsConfig.GetCmsConfigValue("templet_root")).Replace("~", ""));
                ...
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "message", "<script language='javascript' defer>"
                    +
                    "javascript:window.parent.GetClassTemplate('" + path + "','" + this.type + "'); "
                    + "</script>");
